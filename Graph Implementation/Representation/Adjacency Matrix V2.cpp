...
void addEdge(graph* g, int src, int dest){
    g->adjMatrix[src][dest] = 1; // Mark the edge from src to dest
    g->adjMatrix[dest][src] = 1; // Mark the edge from dest to src (for undirected graph)
}

void printAdjMatrix(graph* g){
    for (int i = 0; i < g->V; i++) {
        for (int j = 0; j < g->V; j++) {
            cout << g->adjMatrix[i][j] << " "; // Print the value at the current position in the adjacency matrix
        }
...
int main(){
    ...
    undirectedGraph.adjMatrix = new int*[undirectedGraph.V]; // returns the address of the first element of the array of pointers
    for (int i = 0; i < undirectedGraph.V; i++) {
        undirectedGraph.adjMatrix[i] = new int[undirectedGraph.V]; // returns the address of the first element of the array of integers
    }
    ...
    addEdge(&undirectedGraph, 5, 6);
    addEdge(&undirectedGraph, 6, 5);
    addEdge(&undirectedGraph, 6, 7);
